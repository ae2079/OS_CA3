...
void smoothing(vector<vector<vector<int>>> &image, vector<vector<vector<int>>> &smoothed, int start, int end){
...
void sepia(vector<vector<vector<int>>> &image, vector<vector<vector<int>>> &sepia_out, int start, int end){
  for(int i = start; i < end; i++){
    for(int j = 0; j < cols; j++){
...
int main(int argc, char *argv[])
{
  ...
  for(int i = 0; i < NUM_OF_THREADS; i++){
    ...
    data->out = &sepia_out;
    data->num = i;
    pthread_create(&ptid, NULL, &apply_filters, data);
    ids.push_back(ptid);
  }
  for(int i = 0; i < NUM_OF_THREADS; i++){
    //cout << "here" << i << endl;
  ...
  cout << "Time of Smoothing and Sepia Filter (ms): " << (double)(t3 - t2) / CLOCKS_PER_SEC * 1000 << endl;
